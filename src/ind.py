...
def donchian_channel(df, n, column_name = None):
    ...
    while (i + n - 1) < (len(df.index)-1):
        dc = max(df['high_price'].iloc[i:i + n - 1]) - min(df['low_price'].iloc[i:i + n - 1])
        dc_l.append(dc)
        i += 1
...
"""
def price_volume_trend(df, n, column_name = None):
    if column_name is None:
        column_name = 'close_price'
    ...
    while i < len(df.index):
        last_close = df.loc[df.index[i - 1], column_name]
        today_close = df.loc[df.index[i], column_name]
        today_vol = df.loc[df.index[i], 'volume']
        current_val = current_val + (today_vol * (today_close - last_close) / last_close)
        pvt_l.append(current_val)
        i += 1

    pvt_s = pd.Series(pvt_l).ewm(ignore_na=False, min_periods=0, com=n, adjust=True).mean()
    df['pvt_ema' + str(n)] = pvt_s.values
...
"""
def williams_r(df, period, column_name = None):
    if column_name is None:
        column_name = 'close_price'
    ...
    for row in df.iterrows():
        if idx >= period:
            v = max(df['high_price'].iloc[idx-period:idx]) - df.loc[df.index[idx], column_name]
            v /= (max(df['high_price'].iloc[idx-period:idx]) - min(df['low_price'].iloc[idx-period:idx]))
            wm.append(v)
        idx += 1

    df['williams_r' + str(period)] = pd.Series(wm).values
